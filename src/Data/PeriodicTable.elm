...
type PeriodicTable
    = PeriodicTable (List Atom) (List PTableElem) -- stores the atom for easier access internally
...
findAtom name (PeriodicTable atoms _) =
    Util.find (\atom -> atom.symbol == name) atoms
...
splitUpperLower (PeriodicTable _ elems) =
    List.foldr
        (\elem ( nonFBlocks, fBlocks ) ->
            case elem of
                Atom atom ->
                    if Atom.isFBlock atom then
                        ( nonFBlocks, elem :: fBlocks )
...
fromAtoms atoms =
    List.map Atom atoms
        ++ List.map Placeholder placeholders
        |> PeriodicTable atoms
